...
import { browserSourceSkillDefinitionSchema } from "./source-browser-skills";
...
const sourceAdapterPayloadSchema = title.extend({
  sourceMonitorId: z.uuid(),
  adapterType: z.enum(["rss", "github_markdown", "csv", "greenhouse", "lever", "ashby", "json", "html_selector", "browser_workflow", "browser_skill"]),
  definition: z.record(z.string(), z.unknown()),
  checksum: z.string().min(16),
  domainAllowlist: z.array(z.string()).min(1),
  testResult: z.record(z.string(), z.unknown()).optional(),
}).superRefine((payload, context) => {
  if (payload.adapterType !== "browser_skill") return;
  const parsed = browserSourceSkillDefinitionSchema.safeParse(payload.definition);
  if (!parsed.success)
    context.addIssue({ code: "custom", message: `Invalid browser source skill definition: ${parsed.error.issues.map((issue) => issue.message).join("; ")}` });
});
...
export const mutationRegistry = {
  ...
  "open_question.upsert": define(z.object({ question: z.string(), reason: z.string(), affectedFields: z.array(z.string()) }), owner("career-onboarding", "career-state-curator", "career-positioning", "career-resume-tailor", "career-daily-weekly-planner"), "auto_apply", "upsertOpenQuestion", { question: "Which role track is primary?", reason: "conflict", affectedFields: ["role_targets"] }, "natural_key"),
  ...
  "source_monitor.update": define(target, owner("career-source-discovery", "career-source-adapter-builder", "career-source-browser-reader"), "auto_apply", "updateSourceMonitor", { id: "00000000-0000-4000-8000-000000000001", status: "active" }, "target_version"),
  "source_adapter.upsert": define(sourceAdapterPayloadSchema, owner("career-source-adapter-builder"), "auto_apply", "upsertSourceAdapter", { title: "Greenhouse adapter", sourceMonitorId: "00000000-0000-4000-8000-000000000001", adapterType: "greenhouse", definition: {}, checksum: "0123456789abcdef", domainAllowlist: ["example.com"], testResult: {} }, "natural_key"),
  "signal.create": define(title.extend({ signalType: z.string(), sourceType: z.string(), sourceMonitorId: z.uuid().optional(), sourceRunId: z.uuid().optional(), sourceUrl: z.url().optional(), canonicalUrl: z.url().optional(), externalRef: z.string().optional(), payload: z.record(z.string(), z.unknown()) }), owner("career-source-discovery", "career-source-adapter-builder", "career-source-browser-reader", "career-opportunity-intelligence", "career-event-scanner"), "auto_apply", "createSignal", { title: "Role opened", signalType: "job_post", sourceType: "company_careers_page", sourceUrl: "https://example.com/jobs/1", payload: {} }, "natural_key"),
  ...
  "experience.upsert": define(title.extend({ organization: z.string().optional(), startsAt: z.iso.date().optional(), endsAt: z.iso.date().optional(), description: z.string().optional(), componentType: z.enum(["work", "research", "leadership", "club", "volunteer", "other"]), sourceArtifactId: z.uuid(), sourceSpan: z.record(z.string(), z.unknown()), labels: z.array(z.string()).default([]), metadata: z.record(z.string(), z.unknown()).default({}) }), owner("career-resume-tailor"), "auto_apply", "upsertExperienceComponent", { title: "Software Engineering Intern", organization: "Example", componentType: "work", sourceArtifactId: "00000000-0000-4000-8000-000000000001", sourceSpan: { page: 1 }, labels: [], metadata: {} }, "natural_key"),
  "experience_achievement.upsert": define(z.object({ experienceId: z.uuid(), achievement: z.string().min(1), metrics: z.record(z.string(), z.unknown()).default({}), skillNames: z.array(z.string()).default([]), sourceArtifactId: z.uuid(), sourceSpan: z.record(z.string(), z.unknown()) }), owner("career-resume-tailor"), "auto_apply", "upsertExperienceAchievement", { experienceId: "00000000-0000-4000-8000-000000000001", achievement: "Built an internal automation", metrics: {}, skillNames: [], sourceArtifactId: "00000000-0000-4000-8000-000000000002", sourceSpan: { page: 1 } }, "natural_key"),
  "project.upsert": define(title.extend({ description: z.string().optional(), url: z.url().optional(), repositoryUrl: z.url().optional(), skillNames: z.array(z.string()).default([]), sourceArtifactId: z.uuid(), sourceSpan: z.record(z.string(), z.unknown()), metadata: z.record(z.string(), z.unknown()).default({}) }), owner("career-resume-tailor"), "auto_apply", "upsertProjectComponent", { title: "Career project", description: "Verified project", skillNames: [], sourceArtifactId: "00000000-0000-4000-8000-000000000001", sourceSpan: { page: 1 }, metadata: {} }, "natural_key"),
  "skill.upsert": define(title.extend({ proficiency: z.string().optional(), sourceArtifactId: z.uuid(), sourceSpan: z.record(z.string(), z.unknown()), metadata: z.record(z.string(), z.unknown()).default({}) }), owner("career-resume-tailor"), "auto_apply", "upsertSkillComponent", { title: "TypeScript", sourceArtifactId: "00000000-0000-4000-8000-000000000001", sourceSpan: { page: 1 }, metadata: {} }, "natural_key"),
  "resume_version.update": define(target.extend({ status: z.enum(["processing", "review_required", "verified", "failed", "active"]), metadata: z.record(z.string(), z.unknown()).default({}) }), owner("career-resume-tailor"), "auto_apply", "updateResumeImport", { id: "00000000-0000-4000-8000-000000000001", status: "review_required", metadata: {} }, "target_version"),
  "resume_variant.create": define(title.extend({ templateId: z.uuid().optional(), baseVersionId: z.uuid().optional(), applicationId: z.uuid().optional(), targetRole: z.string().optional(), targetCompany: z.string().optional(), latexPath: z.string().min(1), pdfPath: z.string().optional(), diffPath: z.string().optional(), rationale: z.string().default(""), items: z.array(z.object({ itemType: z.string(), sourceObjectId: z.uuid().optional(), sourceText: z.string(), renderedText: z.string(), rationale: z.string(), position: z.number().int().nonnegative() })), metadata: z.record(z.string(), z.unknown()).default({}) }), owner("career-resume-tailor"), "auto_apply", "createResumeVariant", { title: "Role resume", latexPath: "user/variant/main.tex", pdfPath: "user/variant/main.pdf", items: [], metadata: {} }),
  ...
  "evidence.create": define(title.extend({ sourceType: z.string(), payload: z.record(z.string(), z.unknown()) }), owner(...(["career-advisor", "career-onboarding", "career-state-curator", "career-positioning", "career-source-discovery", "career-source-adapter-builder", "career-source-browser-reader", "career-opportunity-intelligence", "career-application-manager", "career-resume-tailor", "career-relationship-manager", "career-event-scanner", "career-daily-weekly-planner", "career-project-spec"] as AgentId[])), "auto_apply", "createEvidence", { title: "Source evidence", sourceType: "web", payload: {} }),
