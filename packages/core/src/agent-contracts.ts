...
const contracts = {
  ...
  "career-source-browser-reader": { purpose: "Use a source-specific skill to read one configured browser source and return evidence-backed signals.", inputTypes: ["source.browser_read"], outputMutationTypes: ["source_monitor.update", "signal.create", "evidence.create"], allowedCapabilities: [...rw, "browser_read"] },
  ...
  "career-resume-tailor": { purpose: "Import resumes into verified components, then generate and compile opportunity-specific variants.", inputTypes: ["resume.import", "resume.create_variant", "resume.compile", "resume.review"], outputMutationTypes: ["experience.upsert", "experience_achievement.upsert", "project.upsert", "skill.upsert", "resume_version.update", "resume_variant.create", "artifact.create", "open_question.upsert", "task.create", "evidence.create"], allowedCapabilities: [...rw, "read_files", "write_workspace", "latex_compile"] },
