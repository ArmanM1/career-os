...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await context.admin.from("agent_events").insert({ user_id: context.device.userId, agent_run_id: run?.id ?? null, event_type: parsed.data.eventType, message: parsed.data.message, payload: toJson(parsed.data.payload), created_by: "system", updated_by: "system" });
