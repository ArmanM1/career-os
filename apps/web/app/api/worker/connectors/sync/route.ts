...
export async function POST(request: Request) {
  ...
  try { const token = await accessToken(admin, account, credential); const result = account.provider === "gmail" ? await syncGmail(admin, device.userId, token) : account.provider === "google_calendar" ? await syncCalendar(admin, device.userId, token) : await syncGitHub(admin, device.userId, token); await admin.from("connector_sync_runs").update({ status: "success", completed_at: new Date().toISOString(), records_seen: result.seen, signals_created: result.created, updated_by: "system" }).eq("id", run.id); await admin.from("connected_accounts").update({ last_synced_at: new Date().toISOString(), updated_by: "system" }).eq("id", account.id); if (result.created > 0) { const downstream = account.provider === "gmail" ? { agentId: "career-application-manager", inputType: "application.status_check", title: "Review new application email signals" } : account.provider === "google_calendar" ? { agentId: "career-event-scanner", inputType: "event.scan", title: "Review new calendar career events" } : { agentId: "career-positioning", inputType: "positioning.review", title: "Review new GitHub activity" }; await admin.from("agent_jobs").insert({ user_id: device.userId, agent_id: downstream.agentId, title: downstream.title, queue: "connectors", input_type: downstream.inputType, input: { schemaVersion: 1, type: downstream.inputType, connectorSyncRunId: run.id, connectedAccountId: account.id, signalCount: result.created }, related_object_ids: [run.id, account.id], priority: 65, scheduled_for: new Date().toISOString(), dedupe_key: `connector-followup:${run.id}`, created_by: "system", updated_by: "system" }); } return NextResponse.json(result); }
  catch (error) { const message = error instanceof Error ? error.message : String(error); await admin.from("connector_sync_runs").update({ status: "failed", completed_at: new Date().toISOString(), error_message: message, updated_by: "system" }).eq("id", run.id); const needsReauth = /reauthorization|token refresh/i.test(message); if (needsReauth) await admin.from("connected_accounts").update({ status: "needs_reauth", updated_by: "system" }).eq("id", account.id); await enqueueSystemEmail(admin, { userId: device.userId, category: "connector_failure", severity: needsReauth ? "important" : "urgent", idempotencyKey: `connector-failure:${account.id}:${new Date().toISOString().slice(0, 10)}`, subject: `${String(account.provider).replaceAll("_", " ")} connection needs attention`, body: message, actionUrl: "/settings", actionLabel: "Review connection" }); return NextResponse.json({ error: message }, { status: 502 }); }
}
